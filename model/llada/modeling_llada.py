...
from model.llada.configuration_llada import (
...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if config.flash_attention:
            try:
                # first try to use flash_attn 3
                import flash_attn_interface  # type: ignore

                self.flash_attn_func = flash_attn_interface.flash_attn_func
            except Exception:
                # then try to use flash_attn 2
                try:
                    from flash_attn import flash_attn_func  # type: ignore

                    self.flash_attn_func = flash_attn_func
                except Exception:
                    # Fall back to torch, which may also use flash attn if avail.
                    pass
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            # Try FlexAttention if a mask is provided (e.g., to disable self attention).
            # if attn_mask is not None:
            #     try:
            #         raise NotImplementedError
            #         return flex_attention(q, k, v, mask=attn_mask)
            #     except Exception:
            #         # Fallback to PyTorch SDPA below.
            #         pass
    ...
    def attention(
        ...
        if layer_past is not None:
            ...
            if replace_position is None:
                ...
                replace_indices = replace_position.nonzero(as_tuple=True)[
                    1
                ]  # [selected_length]
        ...
        # shape: (B, nh, T, hs)
        # Build a non-causal "no-self" mask: disallow only self-attention (diagonal).
        # Mask shape must be broadcastable to (B, n_heads, query_len, key_len).
        # no_self = torch.zeros((1, 1, key_len, key_len), dtype=q.dtype, device=q.device)
        # diagonal = torch.eye(key_len, dtype=torch.bool, device=q.device).view(1, 1, key_len, key_len)
        # no_self.masked_fill_(diagonal, torch.finfo(no_self.dtype).min)
        # # Align the mask rows to the current query window in case of KV cache usage.
        # attn_mask = no_self[:, :, key_len - query_len : key_len, :key_len]
...
class LLaDABlockDiffBlock(LLaDABlock):
    ...
    def cross_attn_flex(self, qkv, mask=None):
        ...
        x = flex_attention(qkv[:, :, 0], qkv[:, :, 1], qkv[:, :, 2], mask=mask)
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                if output_hidden_states and not final_hidden_state_only:
                    # add hidden states from intermediate layers
        ...
        if output_hidden_states:
            # Only keep the final hidden state when requested, otherwise keep full history.
            if final_hidden_state_only:
                all_hidden_states = [x]
            else:
                all_hidden_states.append(x)
