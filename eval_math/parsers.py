...
class Parser:
    ...
    @classmethod
    def extract_answer_gsm8k_fallback(cls, generated_text):
        """Extract numeric GSM8K answer from multiple patterns."""
        if not generated_text:
            return None
        boxed_pred = cls.extract_answer_boxed(generated_text)
        boxed_value = cls.to_float(boxed_pred)
        if boxed_value is not None:
            return boxed_value

        pound_value = cls.extract_answer_gsm8k(generated_text)
        if pound_value is not None:
            return pound_value

        answer_tag = re.search(r"<answer>(.*?)</answer>", generated_text, re.DOTALL)
        if answer_tag:
            tagged_value = cls.to_float(answer_tag.group(1))
