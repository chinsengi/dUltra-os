#!/bin/bash
#SBATCH --job-name=dultra-math-factor-sweep
...
#SBATCH --output=slurm_out/dultra_math_factor_sweep-%j.out

set -euo pipefail

# Factor sweep for dUltra math models on GSM8K + Math500 using the lladou inference path.
#
# Usage (interactive):
#   bash eval/run_dultra_math_factor_sweep.sh
#
# Common overrides:
#   MODEL_PATH=/path/or/hf/repo MODE=inference BLOCK_LENGTHS="256" FACTORS="0.5 0.8 1.2 1.5" SEEDS="42 123" bash eval/run_dultra_math_factor_sweep.sh
...
DATASET="${DATASET:-math}"            # Use "all" for gsm8k + math500; "math" corresponds to Math500 in eval/eval.py
...
# read -r -a MODELS <<< "${MODELS:-sengi/dUltra-math sengi/dUltra-math-b128}"
read -r -a MODELS <<< "${MODELS:-sengi/dUltra-coding-b128}"
...
#   MODEL_BLOCK_LENGTHS="sengi/dUltra-math=32 sengi/dUltra-math-b128=128"
declare -A MODEL_BLOCK_LENGTH=(
  ["sengi/dUltra-math"]=32
  ["sengi/dUltra-math-b128"]=128
